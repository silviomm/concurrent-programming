...
//Quantidade de caracteres entre '!' e 'z'
#define TAM 90

//Quantidade máxima de caracteres que o fgets irá ler do arquivo
#define LINHA 200
...
void inicializaPossiveisChar(FILE *arq){
  ...
  frequencia = (int *) malloc(sizeof(int)*TAM);
  int i;
  fgetc(arq);
  for(i = 0; i<qntd; i++){
    validos[i] = fgetc(arq);
...
//Checa se o caracter está entre os caracteres '!' e 'z', que são o primeiro e o último que englobam todos os validos
...
void * leitura(void* args){
  FILE *arq = (FILE *) args;
  char lido[LINHA];
  int* freq_aux = (int *) malloc(sizeof(int)*TAM);
  int i;
  for(i=0;i<TAM;i++){freq_aux[i] = 0;}
